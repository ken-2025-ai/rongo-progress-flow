...
import { UserPlus, UploadCloud, GraduationCap, BookOpen } from "lucide-react";
...
const STUDY_LEVELS = [
  { value: "phd", label: "Doctor of Philosophy (PhD)", icon: "🎓", keywords: ["phd", "doctor"] },
  { value: "masters", label: "Master's Degree (MSc / MA)", icon: "📚", keywords: ["msc", "ma.", "master"] },
];
...
export function StudentRegistry() {
  ...
  // Filter programmes by dept + study level
  const filteredProgrammes = programmes.filter(p => {
    const matchesDept = !selectedDept || p.department_id === selectedDept;
    const matchesLevel = !selectedLevel || STUDY_LEVELS.find(l => l.value === selectedLevel)
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5 col-span-2">
                  ...
                  >
                    ...
                    {/* Group by keyword */}
                    {selectedLevel === 'phd' && filteredProgrammes.length > 0 && (
                    ...
                    {selectedLevel === 'masters' && filteredProgrammes.length > 0 && (
                    ...
                    {!selectedLevel && filteredProgrammes.map(p => (
                      <option key={p.id} value={p.id}>{p.name}</option>
                    ))}
                  </select>
                </div>
          ...
          <div className="card-shadow rounded-2xl bg-card border border-border shadow-sm overflow-hidden">
            ...
            <div className="p-5 space-y-3 text-xs">
              {[
                ...
                { label: "School", value: selectedSchool ? schools.find(s => s.id === selectedSchool)?.name : "—" },
                { label: "Department", value: selectedDept ? departments.find(d => d.id === selectedDept)?.name : "—" },
                { label: "Programme", value: selectedProg ? filteredProgrammes.find(p => p.id === selectedProg)?.name : "—" },
              ].map(item => (
