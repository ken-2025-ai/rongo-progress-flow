...
const DECISION_LOGS = [
  { 
    ...
    decision: "Pass",
    recordedBy: "Dr. Ochieng (Coordinator)"
  },
  { 
    id: 2, 
    date: "March 15, 2026", 
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 bg-card p-4 rounded-xl border border-border/50">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">Immutable audit trail of all recorded seminar and presentation decisions.</p>
        </div>
      ...
      <motion.div variants={itemVariants} className="card-shadow bg-card rounded-xl overflow-hidden border border-border">
         ...
         <Table>
           <TableHeader className="bg-background">
             <TableRow>
               <TableHead className="font-bold">Date Recorded</TableHead>
               <TableHead className="font-bold">Student Identity</TableHead>
               <TableHead className="font-bold">Seminar Level</TableHead>
               <TableHead className="font-bold text-center">Final Consensus</TableHead>
               <TableHead className="text-right font-bold">Recorded By</TableHead>
             </TableRow>
           </TableHeader>
           <TableBody>
               ...
               <TableRow key={log.id}>
                 ...
                 <TableCell className="text-center">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase inline-flex items-center gap-1 border ${
                       log.decision === "Pass" ? "bg-success/10 text-success border-success/20" : 
                       log.decision === "Minor Corrections" ? "bg-status-warning/10 text-status-warning border-status-warning/20" : 
                       log.decision === "Major Corrections" ? "bg-destructive/10 text-destructive border-destructive/20" :
                       "bg-muted text-foreground border-border"
                    }`}>
                       {log.decision === "Pass" && <CheckCircle2 size={10} />}
                       {log.decision === "Minor Corrections" && <AlertTriangle size={10} />}
                       {log.decision === "Major Corrections" && <AlertTriangle size={10} />}
                       {log.decision === "Repeat Seminar" && <PlayCircle size={10} />}
                       {log.decision}
                    </span>
                 </TableCell>
                 <TableCell className="text-right">
                    <span className="text-xs font-semibold text-muted-foreground flex items-center justify-end gap-1.5">
                       <Shield size={12} className="text-primary/50" /> {log.recordedBy}
                    </span>
                 </TableCell>
               </TableRow>
             ))}
